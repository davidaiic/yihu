# tools/weda_docgen.py  (v3)
# 特性：
#  - 二次生成仅替换 auto 区块，保留你在 keep 区块里的手写内容
#  - 扫描 miniapp 页面：handlers、callFunction 云函数名、（页面端）db.collection 名
#  - 扫描 cloudfunctions/*/index.js：db.collection 名、cloud.callFunction 名
#  - 产出/更新：
#      * docs/pages/<pageId>/spec.md（含 auto/keep）
#      * cloudfunctions/<fn>/README.md（含 auto/keep，新增 collections-used / calls 区块）
#      * docs/INVENTORY.md
#      * docs/_chatgpt-index.yml
#      * docs/collections/DISCOVERED.md（集合使用关系总览）
#  - 可选 --seed-collections：为新发现的集合创建最小 schema/sample（不覆盖已有）
#
# 依赖：pip install pyyaml
#
# 用法：
#   python tools/weda_docgen.py --src miniapp --cfs cloudfunctions --out . --entry u_shou_hu_home --seed-collections
#
...
def main():
    ...
    for name in sorted(set(list(existing_schemas.keys()) | set(existing_samples.keys()))):
        entry = {}
    ...
    print("已更新：")
    print("  - docs/pages/*/spec.md（仅替换 auto 区块）")
    print("  - cloudfunctions/*/README.md（仅替换 auto 区块，新增 collections-used/calls）")

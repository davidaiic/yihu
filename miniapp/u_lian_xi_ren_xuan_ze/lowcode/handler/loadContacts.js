...
export default async function({event, data}) {
  ...
  try {
    ...
    if (res.result?.data) {
      $page.setState({ contacts: res.result.data });
    } else {
      $page.setState({ contacts: [] });
    }
  } catch (e) {
    console.error('[TODO-REMOVE] loadContacts error', e);
    this.$WEAPPS_COMP.__internal__?.$w?.utils.showToast({ title: '获取联系人失败', icon: 'error' });
  } finally {
    //wx.hideLoading?.();

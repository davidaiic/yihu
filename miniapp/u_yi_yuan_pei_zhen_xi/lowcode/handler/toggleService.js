...
_opts => {
  ...
  const services = [...$comp.dataset.state.services];
  const index = services.indexOf(params.params.service);
  if (index === -1) {
    services.push(params.params.service);
  } else {
    services.splice(index, 1);
  }
  this.$WEAPPS_COMP.dataset.state.services = services;
}
)
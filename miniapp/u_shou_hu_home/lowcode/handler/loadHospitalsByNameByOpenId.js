...
export default async function({event, data}) {
  ...
  const keyword  = this.$WEAPPS_COMP.dataset.state.searchKeywordlast;

  const currentPage = this.$WEAPPS_COMP.dataset.state.currentPage || 1;
  const pageSize = this.$WEAPPS_COMP.dataset.state.pageSize || 5;

  if (!keyword) {
    this.$WEAPPS_COMP.__internal__?.$w?.utils.showToast({ title: '请输入搜索关键词', icon: 'none' });
    return;
  }
  ...
  try {
    const res = await this.$WEAPPS_COMP.__internal__?.$w?.cloud.callFunction({
      ...
      data: { keyword, page: currentPage, pageSize }
    });

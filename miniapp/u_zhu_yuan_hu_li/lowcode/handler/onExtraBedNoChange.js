...
export default function({event, data}) {
  ...
  const newNo = (event.detail?.value || '').trim();     // 微信 input 取值方式:contentReference[oaicite:4]{index=4}
  this.$WEAPPS_COMP.__internal__?.$w?.page.setState({ extraBedNo: newNo });
  ...
  const cached   = allCache[tenantId] || {};
  allCache[tenantId] = { ...cached, extraBedNo: newNo };

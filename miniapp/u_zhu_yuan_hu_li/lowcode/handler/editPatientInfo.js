...
_opts => {
  ...
  // ① 出发前缓存
  const { selectedWardId, selectedRoomId, selectedBedId,
        wardOptions, roomOptions, bedOptions } = this.$WEAPPS_COMP.dataset.state;

  this.$WEAPPS_COMP.__internal__?.$w?.app.setState({
    cache_zhuyuanhuli_order: {
      selectedWardId, selectedRoomId, selectedBedId,
      wardOptions, roomOptions, bedOptions
    }
  });

  this.$WEAPPS_COMP.__internal__?.$w?.utils.navigateTo({
    "pageId": "patient-info-edit"
  });
